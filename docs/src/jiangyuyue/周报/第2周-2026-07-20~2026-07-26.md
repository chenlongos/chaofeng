...
同一张图（tennis-ball-close.jpg）在不同系统/引擎下的推理耗时：
...
V2 速度领先 4.8 倍。原因：V2 模型 anchor 数缩减至 1/3，推理负载显著降低。YUYV native 预处理也贡献了更高检出置信度（无 OpenCV JPEG 压缩损失）。
...
| v2 | v4 融合版（消除 malloc） | 143ms | 79ms | 2.8 | 质的飞跃 |
