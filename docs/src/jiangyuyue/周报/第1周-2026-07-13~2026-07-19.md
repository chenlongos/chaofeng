...
| 系统 | 引擎 | 语言 | 推理耗时 | 加速比 |
|------|------|------|---------|--------|
| **TGOSKits StarryOS** | **TPU** | **Python** | **40ms** | **355x** |
| **TGOSKits StarryOS** | **TPU** | **C++** | **40ms** | **355x** |
...
v3 版本直接输出 TPU 原始结果 `[1,5,8400]` FP32，同一物体被检出 11 个重复框。v4 实现标准 IoU NMS（阈值 0.45）：置信度过滤 → 排序 → 逐框 IoU 抑制，最终精准检出 1 框，与 C++ (40ms) / Rust (41ms) 输出完全对齐。
